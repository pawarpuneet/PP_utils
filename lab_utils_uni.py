""" 
lab_utils_uni.py
    routines used in Linear Regression dealing with single variables (univariate)
"""
import numpy as np
...
from PP_utils.lab_utils_common import compute_cost
from PP_utils.lab_utils_common import dlblue, dlorange, dldarkred, dlmagenta, dlpurple, dlcolors
...
def plt_house_x(X, y,f_wb=None, ax=None):
    ...
    ax.set_title("x_train vs y_train")
    ax.set_ylabel('y_train')
    ax.set_xlabel(f'x_train')
    if f_wb is not None:
...
def plt_intuition(x_train, y_train, wrange=[0,100], tmp_b=0):

    w_range = np.array(wrange)
    tmp_b = tmp_b
    ...
    def func( w=150):
        ...
        ax[1].set_title(f"Cost vs. w, (b fixed at {tmp_b})")
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            d = self.ax[1].annotate(f"Cost: {cst:.0f}, w={round(ws,2)}, b={round(bs,2)}", xy= (ws, bs), xytext = (4,4), textcoords = 'offset points',
...
def soup_bowl(weights=[1,1]):
    ...
    for x in w:
        i=0
        for y in b:
            z[i,j] = weights[0]*x**2 + weights[1]*y**2
            i+=1
        j+=1
    ...
    ax.set_title(f"$J(w,b) = {weights[0]}w^2 + {weights[1]}b^2$\n [You can rotate this figure]", size=15)
